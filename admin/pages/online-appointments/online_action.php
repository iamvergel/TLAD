...
if (isset($_POST['checking_editbtn'])) {
  $s_id = $_POST['app_id'];
  $payment = isset($_POST['payment']) ? $_POST['payment'] : 'N/A';
...
if (isset($_POST['update_appointment'])) {
  ...
  $sql = "UPDATE tblappointment SET doc_id='$doctor_id', schedule='$schedule', starttime='$s_time', endtime='$e_time', reason='$treatment', status='$status', bgcolor='$bgcolor', payment='$payment' WHERE id='$id' ";
  $query_run = mysqli_query($conn, $sql);
  ...
  if ($query_run) {
    if ($status == 'Confirmed') {
      $text = 'Your appointment has been confirmed. The payment amount is: ' . $payment . '. Please try to arrive 10-15 minutes early and don\'t forget to wear your face mask. If you have any queries, or need to reschedule, please call our office at ' . $system_contact . ' or drop us a mail ' . $system_email . '. We look forward to seeing you on ' . $patient_date . ' ' . $patient_time . '. ';
              ...
              <div class="row">
                <div class="col-md-8">
                  <div class="invoice p-3 mb-3" id="prescription">
                    ...
                    <p>Treatment: ' . $treatment . '</p>
                    <p>Payment Amount: <b>' . $payment . '</b></p>
                    Health Declaration';
      while ($row = mysqli_fetch_array($query_run)) {
        $data .= '<ul><li>' . $row['questions'] . ' <b>' . $row['answer'] . '</b></li></ul>';
      }
      $data .= '
                    <br>
                    <table class="table" style="width:100%;">
    ...
    $_SESSION['error'] = "Online Appointment Request Failed to Schedule";
    header('Location:index.php');
  }
}
